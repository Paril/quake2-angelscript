...
void ClientThink(edict_t *ent, usercmd_t *ucmd)
{
	...
	static std::array<std::chrono::nanoseconds, 1000> points;
	static size_t z = 0;
	auto start = clk.now();
#endif
	...
	if (z == 0)
	{
		std::chrono::nanoseconds min = std::chrono::nanoseconds::max(), max = std::chrono::nanoseconds::min(), avg = {};
