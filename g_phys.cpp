...
static void SV_CheckProjectileDodge(edict_t *ent)
{
    // find a monster
    ...
    if (tr.fraction == 1.0f)
        ...
        return;
    else if (tr.ent->deadflag)
        return;
	// we recently made a valid dodge, don't try again for a bit
	else if (tr.ent->monsterinfo.dodge_time > level.time)
		return;
    // no dodge
    else if (!tr.ent->monsterinfo.dodge)
        return;
    else if (!projectile_infront(tr.ent, ent))
        return;

	vec3_t v = tr.endpos - ent->s.origin;
	gtime_t eta = gtime_t::from_sec(v.length() / ent->velocity.length());

	tr.ent->monsterinfo.dodge(tr.ent, ent->owner, eta, &tr, (ent->movetype == MOVETYPE_BOUNCE || ent->movetype == MOVETYPE_TOSS));
}
