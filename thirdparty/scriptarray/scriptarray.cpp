...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	// shuffle
	r = engine->RegisterObjectMethod("array<T>", "void shuffle(uint startAt = 0, uint count = uint(-1))", asMETHODPR(CScriptArray, Shuffle, (asUINT, asUINT), void), asCALL_THISCALL); assert(r >= 0);
...
void CScriptArray::Shuffle(asUINT startAt, asUINT count)
{
	if (startAt >= GetSize())
		return;

	asUINT end = startAt + count;
	...
	static std::mt19937 r((asUINT) std::chrono::system_clock::now().time_since_epoch().count());

	for (int64_t i = GetSize() - 1; i > 0; i--)
	{
		const asUINT j = std::uniform_int_distribution<asUINT>(0, i)(r);
		Swap(GetArrayItemPointer(i), GetArrayItemPointer(j));
	}
}
...
static void ScriptArrayShuffle_Generic(asIScriptGeneric *gen)
{
	asUINT startAt = gen->GetArgDWord(0);
	asUINT count = gen->GetArgDWord(1);
	CScriptArray *self = (CScriptArray*)gen->GetObject();
	self->Shuffle(startAt, count);
}
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "void shuffle(uint startAt = 0, uint count = uint(-1))", asFUNCTION(ScriptArrayShuffle_Generic), asCALL_GENERIC); assert(r >= 0);
#if AS_USE_STLNAMES != 1 && AS_USE_ACCESSORS == 1
